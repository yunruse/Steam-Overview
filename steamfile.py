...
if os.name == 'nt':
    ...
    try:
        ...
        driveLetters = [
            a + ':/' for a in string.ascii_uppercase if _doesDriveExist(a)]
...
class Drive:
    __slots__ = ['path', 'games', 'sizeTotal',
                 'sizeUsed', 'sizeFree', 'sizeGames']

    def __repr__(self):
        return '<Library {!r} ({} games, {}{})>'.format(
            self.path, len(self.games),
            "~" * any(game.size is None for game in self.games),
            bytesize(sum(game.size or game.sizeEstimate for game in self.games), 2, binary=True))

    def __init__(self, path):
        p = Path(path)
        self.path = p.drive or p.root
        self.games = []
        self.sizeTotal, self.sizeUsed, self.sizeFree = shutil.disk_usage(
            str(path))
        self.sizeGames = 0
